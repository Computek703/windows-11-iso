        ...
        if ($vols) {
            foreach ($v in $vols) {
                ...
                elseif ($prot -eq 'Off' -or $state -match "FullyDecrypted") {
                    ...
                    Write-Host "[INFO] BitLocker unknown state on $($v.MountPoint) ($state)" -ForegroundColor Blue
                }
            }
        } else {
            Write-Host "[INFO] No BitLocker volumes found." -ForegroundColor Blue
        }
    }
    catch {
